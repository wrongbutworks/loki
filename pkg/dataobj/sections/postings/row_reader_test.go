...
func TestRowReader_RoundTrip(t *testing.T) {
	...
	for _, s := range obj.Sections() {
		...
		reader := postings.NewRowReader(ctx, sec, nil, nil)
		for reader.Next() {
...
func TestRowReader_BloomMatchPredicate(t *testing.T) {
	...
	countMatches := func(value string) int {
		rr := postings.NewRowReader(ctx, sec, []postings.Predicate{postings.BloomMatchPredicate{Column: bloomCol, Value: []byte(value)}}, nil)
...
func TestRowReader_RegexMatchPredicate(t *testing.T) {
	...
	countMatches := func(pattern string) int {
		...
		require.NoError(t, err)
		rr := postings.NewRowReader(ctx, sec, []postings.Predicate{postings.RegexMatchPredicate{Column: lvCol, Matcher: re}}, nil)
		defer func() { _ = rr.Close() }()
		var got int
		for rr.Next() {
			got++
